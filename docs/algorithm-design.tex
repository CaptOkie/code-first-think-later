...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Description & Report writing is typically seen as a crucial University skill to have. For the purpose of Software Engineering, it becomes important when it comes time to start writing up the deliverables. Knowing how to punctuate, as well as how to structure sentences, are vitally important.\\
		Scale for Responses &
		\begin{minipage}[t]{\linewidth}
			\begin{enumerate}
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Description & It's nearly impossible that a given software runs flawlessly the first time it gets run. Debugging software is something of an art! \\
		Scale for Responses &
		\begin{minipage}[t]{\linewidth}
			\begin{enumerate}
