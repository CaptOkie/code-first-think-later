...
To store the data for our system, a relational database is being used. Our relational database is built in the SQLite architecture. Multiple tables are used to store related information together, such as the \textit{Users} table.
...
\begin{tabu} to \textwidth {l >{\it}l X}
	...
	\cdlabel{storage}\cdref{storage} & Storage & The Storage class is the class that interacts directly with the relational database. All its methods revolve around pulling information from the database, or pushing information into the database. This class feeds the persistent information to other classes so that they can modify/display them, before receiving modified/new information to save into the database.\\
	...
	\cdlabel{response}\cdref{response} & Response & desc \\
