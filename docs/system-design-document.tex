...
Our system revolves heavily upon a central storage, or respository. Other than the repository, we make use of several subsystems. Simply with this in mind, we can instantly rule out the {\it pipe and filter} achitectural style - while we do indeed have subsystems (filters) that associate with each other (pipes), our subsystems are fairly dependent on one another. Furthermor, our subsystems are responsible for more than simply processing a set of inputs to get a set of outputs. 
...
\begin{tabu} to \textwidth {l >{\it}l X}
	...
	\cdlabel{student}\cdref{student} & Student & desc \\
	\cdlabel{admin}\cdref{admin} & Admin & desc \\
	\cdlabel{user}\cdref{user} & User & desc \\
	\cdlabel{storage}\cdref{storage} & Storage & desc \\
	\cdlabel{question}\cdref{question} & Question & desc \\
	\cdlabel{response}\cdref{response} & Response & desc \\
	\cdlabel{answer}\cdref{answer} & Answer & desc \\
	\cdlabel{project}\cdref{project} & Project & desc \\
	\cdlabel{groupsize}\cdref{groupsize} & GroupSize & desc \\
