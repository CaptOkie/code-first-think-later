...
To ensure the ID of a \textit{Student} (\cdref{student}) is unique, the \textit{id} column is set as the primary key. This guarantees that no two IDs can be equal.
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l l l X}
		...
		id   & Integer & Primary Key & The unique ID given to the Student (\cdref{student}). \\
		name & Text    &             & The name of the Student (\cdref{student}). \\
	\end{tabu}
\end{table}

\subsubsection*{Administrator Table (\dtref{admins})}
To ensure the ID of an \textit{Admin} (\cdref{admin}) is unique, the \textit{id} column is set as the primary key. This guarantees that no two IDs can be equal. As well, by having the \textit{id} column as the primary key, the speed of looking up an \textit{Admin} by their ID is increased.
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l l l X}
		\tableheader{}Column & Type & Constraint & Description \\
		id   & Integer & Primary Key & The unique ID given to the Admin (\cdref{admin}). \\
		name & Text    &             & The name of the Admin (\cdref{admin}). \\
	\end{tabu}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l l X X[3]}
		\tableheader{}Column & Type & Constraint & Description \\
		id             & Integer & Primary Key      & The unique ID given to a Project (\cdref{project}). \\
		name           & Text    & Unique,\newline 
		                           Case Insensitive & The unique name given to a Project (\cdref{project}). \\
		max\_grp\_size & Integer &                  & The maximum group size of a Project (\cdref{project}). \\
		min\_grp\_size & Integer &                  & The minimum group size of a Project (\cdref{project}). \\
	\end{tabu}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l l l X}
		\tableheader{}Column & Type & Constraint & Description \\
		id       & Integer & Primary Key & The unique identifier given to a Question (\cdref{question}). \\
		personal & Text    &             & The text for the question asking the Student (\cdref{student}) to describe their self. \\
		desired  & Text    &             & The text for the question asking the Student (\cdref{student}) to described their desired partner. \\
		category & Text    &             & The qualification category the Question (\cdref{question}) belongs to. \\
	\end{tabu}
\end{table}

\subsubsection*{Answers Table (\dtref{answers})}
In the system, an \textit{Answer} (\cdref{answer}) appears only once in a \textit{Question} (\cdref{question}), therefore the database should model that. To accomplish this, the primary key of the \textit{answers} table is made up of two columns: \textit{question}, which references the \textit{id} from the \textit{questions} table (\dtref{questions}), and \textit{id}, which is a unique ID for each \textit{Answer}. By having both columns as the primary key, this ensures that only one combination of \textit{question} and \textit{id} exist. That means a \textit{Question} can only have one occurrence of an \textit{Answer}.
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l l X X[4]}
		\tableheader{}Column & Type & Constraint & Description \\
		stu      & Integer & Primary Key,\newline
		                     Foreign Key          & Identifies the associated Student (\cdref{student}) with the response. References \textit{id} of the 
		                                            \textit{students} table. \\
		question & Integer & Primary Key,\newline
		                     Foreign Key          & Identifies which Question (\cdref{question}) the response belongs to. References \textit{question}
		                                            of the \textit{answers} table. \\
		personal & Integer & Foreign Key          & Identifies a Student's  (\cdref{student}) selection for their personal attribute for the 
		                                            given Question (\cdref{question}). References \textit{id} of the \textit{answers} table. \\
		desired  & Integer & Foreign Key          & Identifies a Student's (\cdref{student}) selection for their desired attribute for the 
		                                            given Question (\cdref{question}). References \textit{id} of the \textit{answers} table. \\
	\end{tabu}
\end{table}
...
\subsubsection*{Proxy}
We have chosen to implement this design pattern in our system. By encompassing several of our entity objects in a proxy design, we can implement a `lazy' load-type system in that the system will only load the information that we directly ask for - nothing extra. This has the enormous benefit of reducing the memory footprint. The time trade-off is comparably minimal, even negligible. Furthermore, with this design pattern in place, the control objects do not need to interact with the Storage when performing updates and retrieval. The following figure, \figureref{proxy-example}, provides a small UML-based visual of how the Proxy Pattern is set up. The idea is that the ProxyObject acts as a kind of interface to the RealObject, wherein the rest of the system has no access to the RealObject.
...
\begin{tabu} to \textwidth {l >{\it}l X}
	    \tableheader{}ID & Class Name & Description \\
	\cdlabel{student-form}\cdref{student-form} & StudentForm & The StudentForm class is a boundary object - the only operation that this class has is show. This class displays the information concerning the particular Student User that is currently logged in.\\
	    \cdlabel{admin-form}\cdref{admin-form} & AdminForm & The AdminForm class is a boundary object - the only operation that this class has is show. This class displays the information concerning the particular Admin User that is currently logged in.\\
	\end{tabu}
\end{center}
...
The following table, \tableref{profile-option-classes}, describes each of the classes involved in the ProfileOptionsService service (\ssref{profile-options}) as well as provide a unique identifier for each.

\begin{table}[H]
	\caption{ProfileOptionsService Classes (\ssref{profile-options})} \tablelabel{profile-option-classes}
	\begin{tabu} to \textwidth {l >{\it}l X}
	    \tableheader{}ID & Class Name & Description \\
		\cdlabel{profile-form}\cdref{profile-form} & ProfileForm & The ProfileForm class is a boundary object class - the only operation that this class has is show. This class displays the information concerning the particular Student User that is currently logged in's Profile.\\
...
The following table, \tableref{project-option-classes}, describes each of the classes involved in the ProjectOptionsService service (\ssref{project-options}) as well as provide a unique identifier for each.
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l >{\it}l X}
		...
		\cdlabel{project-form}\cdref{project-form} & ProjectForm & The ProjectForm class is a boundary object - the only operation that this class has is show. This class displays the information concerning the particular Admin User that is currently logged in's Projects.\\
	\end{tabu}
\end{table}
...
The following table, \tableref{ppid-option-classes}, describes each of the classes involved in the PPIDOptionsService service (\ssref{ppid-options}) as well as provide a unique identifier for each.
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l >{\it}l X}
		...
		\cdlabel{ppid-form}\cdref{ppid-form} & PPIDForm & The PPIDForm class is a boundary object - the only operation that this class has is show. This class displays the information concerning the last run PPID algorithm for a specific Project.\\
	\end{tabu}
\end{table}
...
The following table, \tableref{user-classes}, describes each of the classes involved in the UserService service (\ssref{user}) as well as provide a unique identifier for each.
...
The following table, \tableref{student-classes}, describes each of the classes involved in the StudentService service (\ssref{student}) as well as provide a unique identifier for each.
...
The following table, \tableref{project-classes}, describes each of the classes involved in the ProjectService service (\ssref{project}) as well as provide a unique identifier for each.
...
\begin{table}[H]
    \caption{Project Class (\cdref{project})} \tablelabel{project}
	\begin{tabu} to \textwidth {>{\it}l X}
		\toprule
		...
		\begin{minipage}[t]{\linewidth}
	        ...
	        \end{itemize}
	    \end{minipage} \\
	    	Traceability & \ssref{project}, \opref{project-get-students}, \opref{project-get-groups}, \opref{project-set-groups}, \sdref{project}\\
		\toprule
	\end{tabu}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		\begin{minipage}[t]{\linewidth}
	        ...
	        \end{itemize}
	    \end{minipage} \\
	    	Traceability & \ssref{project}, \sdref{project}\\
		\toprule
	\end{tabu}
\end{table}
...
The following figure, \figureref{project-class-diagram}, is a UML Class Diagram for the classes involved in the ProjectService service (\ssref{project}).
...
The following table, \tableref{question-classes}, describes each of the classes involved in the QuestionService service (\ssref{question}) as well as provide a unique identifier for each.
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		\begin{minipage}[t]{\linewidth}
	        ...
	        \end{itemize}
	    \end{minipage} \\
	    	Traceability & \ssref{question}, \opref{question-get-answers}, \opref{question-get-personal-answer}, \opref{question-get-desired-answer}, \opref{question-set-personal}, \opref{question-set-desired}, \sdref{storage}\\
		\toprule
	\end{tabu}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		\begin{minipage}[t]{\linewidth}
	        ...
	        \end{itemize}
	    \end{minipage} \\
	    	Traceability & \ssref{question}, \sdref{storage}\\
		\toprule
	\end{tabu}
\end{table}
...
\newpage{}
The following figure, \figureref{question-class-diagram}, is a UML Class Diagram for the classes involved in the QuestionService service (\ssref{question}).
