...
\subsection*{Objects in Storage}
\begin{table}[H]
	\caption{Objects in Storage} \tablelabel{storage-obj}
	\begin{tabu} to \textwidth {l X}
		\tableheader{}ID & Name & Description \\
		\dtlabel{students}\dtref{students}     & Student   & Stores the Student entity object. \\
		\dtlabel{admins}\dtref{admins}         & Admin     & Stores the Admin entity object. \\
		\dtlabel{projects}\dtref{projects}     & Project   & Stores the Project entity object. \\
		\dtlabel{questions}\dtref{questions}   & Question  & Stores the Question entity object. \\
		\dtlabel{answers}\dtref{answers}       & Answer    & Stores the Answer entity object. \\
	\end{tabu}
\end{table}
...
- unique project name, can't have projects of same name \\
Oftentimes, admins will have to create multiple projects. To assure that no duplicate projects exist, we have enforced a unique project name attribute. This assures that the admin won't confuse themselves by having several identically named projects, thus maximizing readabilit and useability.

\subsubsection*{enrollment}
- primary key made up of \textit{students}.\textit{id} and \textit{projects}.\textit{name} \\
- means can only have one entry per student and project association \\
...
In this section, we give a list and description of each of the services that our subsystems offer. They will be divided into three separate tables - one for each of our subsystems. \sdlabel{proxy}The following table encompasses all the services offered by the Proxy Subsystem (\sdref{proxy}). 

\begin{table}[H]
\caption{Services Offered by the Proxy Subsystem (\sdref{proxy})} \tablelabel{service-sub-proxy}
\begin{tabu} to \textwidth {l >{\it}l X}
...
\sdlabel{ppid}The following table showcases all the different services offered by the PPID Subsytem (\sdref{ppid}).

\begin{table}[H]
\caption{Services Offered by PPID Subsystem (\sdref{ppid})} \tablelabel{service-sub-ppdi}
\begin{tabu} to \textwidth {l >{\it}l X}
	\tableheader{}ID & Service & Description\\
