...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l l X X[4]}
		...
		project & Text    & Primary Key,\newline
		                    Case Insensitive     & References \textit{name} of the \textit{projects} table. \\
	\end{tabu}
\end{table}
...
While it it, in the real world, possible for multiple people to share the same name, we want to make certain that the system can handle them appropriately. To do so, we added a unique identifier attribute, which is independent of the name itself. With this kind of system, it is impossible for duplicate accounts to exist.

\subsubsection*{admins}
Similar to the student duplicate account possibility, we have implemented the same unique identifier system for admin users. Additionally, it is important to note that both admins and students share the same numbering scheme for ID's, so it is impossible for a student and an admin to share the same ID.
