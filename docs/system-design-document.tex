...
Its important that each student can only enroll in each project once. To accomplish this, as shown in table \tableref{enrollment-table}, the \textit{enrollment} table has two columns: \textit{stu}, to reference the \textit{id} column for \textit{students}, and \textit{project}, to reference \textit{id} from projects. The primary key is then made up of both columns to ensure that only one instance of a \textit{students} entry and \textit{projects} entry combination exists at one time.
...
A \textit{Student} is going to have one, and only one, response for every \textit{Question}. In that response, a \textit{Student} has a desired and personal value. To model this relationship, as shown in table \tablelabel{responses-table}, \textit{responses} table has a primary key made up of two column: \textit{stu}, which references \textit{id} from the \textit{students} table (\dtref{students}), and \textit{question}, which references \textit{id} from the \textit{questions} table (\dtref{questions}). This ensures that a \textit{Student} can have only one response for a given \textit{Question}.
