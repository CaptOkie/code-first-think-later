...
The design patterns in this section affect how objects get created. There will be two patterns in this section that we will be considering for our system: the factory method design pattern and the abstract factory design pattern. 
\subsubsection*{Abstract Factory}
While we were potentially considering implementing this design pattern, we decided not to in the end in favor of the more directly applicable Factory Method pattern. This design patterns essentially works by providing a standard interface for creating objects, wherein the concrete classes themselves are not specified. This would not work well for our system since it is crucial for objects to know whether the class is a Student user or an Admin user - functionality is highly dependent on this fact.
\subsubsection*{Factory Method}
We have decided to implement this design pattern in our system. The idea for this pattern is define a specialized interface for object creation that lets subclasses make the decision as to which particular class to instantiate. This pattern is apparent a user attempts to login - we do not know initially what type of user is logging in. In this scenario, we let UserStorage class tell us what type of user it is and then the UserControlFactory class will instantiate it. Another way of seeing this pattern is to imagine the UserControlFactory as a Template method - there is a placeholder of sorts that subclasses will specify.
