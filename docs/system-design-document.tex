...
Each \textit{Project} object (\cbref{project}) has a unique ID to uniquely identify it when querying or for referencing it in other tables. This ID is stored in the \textit{id} column, which is the primary key for the table. \textit{id} is used as the primary key oppose to the \textit{name} column, even though \textit{name} is always going to be unique as well, due to the fact the column is being referenced in other tables. If a \textit{Project's} name is changed, not only must be changed in the \textit{name} column, but in every table it is referenced as well. That operation can be time consuming and, most importantly, may lead to errors causing the data in the database to be incorrect. By using an ID that will never change, then the name of the \textit{Project} can change without fear of causing corrupt data.

The following table, \tablelable{projects-table}, gives a more detailed breakdown and description of each of the columns of the \textit{projects} table.
...
Its important that each student can only enroll in each project once. To accomplish this, we have two Primary Keys - the student's unique identitier (\textit{students}.\textit{id}) and the project's unique name (\textit{projects}.\textit{name}). The following table, \tableref{enrollment-table}, gives a more detailed breakdown and description of each of the columns of the \textit{enrollment} table.
...
\begin{tabu} to \textwidth {l >{\it}l l X}
	...
	\oplabel{getstuname}\opref{getstuname} & GetStudentName & ProxyStudent (\cdref{proxystudent}) & The \textit{GetStudentName} operation is a method offered by the \textit{ProxyStudent} class (\cdref{proxystudent}). The operation takes in an ID value (an integer), and asks the \textit{RealStudent} class (\cdref{realstudent}) for the \textit{Student} corresponding to the given ID's name and returns it (as a String).\\
	\oplabel{getadminname}\opref{getadminname} & GetAdminName & ProxyAdmin (\cdref{proxyadmin}) & The \textit{GetAdminName} operation is a method offered by the \textit{ProxyAdmin} class (\cdref{proxyadmin}). The operation takes in an ID value (an integer), and asks the \textit{RealAdmin} class (\cdref{realadmin}) for the \textit{Admin} corresponding to the given ID's name and returns it (as a String).\\
	\oplabel{getprojectname}\opref{getprojectname} & GetProjectName & ProxyProject (\cdref{proxyproject}) & The \textit{GetProjectName} operation is a method offered by the \textit{ProxyProject} class (\cdref{proxyproject}). The operation takes in an ID value (an integer) and asks the \textit{RealProject} class (\cdref{realproject}) for the \textit{Project} corresponding to the given ID's name and returns it (as a String).\\
	\oplabel{getdesiredquestion}\opref{getdesiredquestion} & GetDesiredQuestion & ProxyQuestion (\cdref{proxyquestion}) & desc\\
	\oplabel{getpersonalquestion}\opref{getpersonalquestion} & GetPersonalQuestion & ProxyQuestion (\cdref{proxyquestion}) & desc\\
	\oplabel{getcategory}\opref{getcategory} & GetQuestionCategory & ProxyQuestion (\cdref{proxyquestion}) & desc\\
	\oplabel{getmaxgroupsize}\opref{getmaxgroupsize} & GetMaxGroupSize & ProxyProject (\cdref{proxyproject}) & desc\\
	\oplabel{getmingroupsize}\opref{getmingroupsize} & GetMinGroupSize & ProxyProject (\cdref{proxyproject}) & desc\\
\end{tabu}
\end{table}
