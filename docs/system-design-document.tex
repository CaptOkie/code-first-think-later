...
\begin{tabu} to \textwidth {l >{\it}l l X}
	...
	\oplabel{getstuname}\opref{getstuname} & GetStudentName & ProxyStudent & The \textit{GetStudentName} operation is a method offered by the \textit{ProxyStudent} class (\cdref{proxystudent}). The operation takes in an ID value (an integer), and asks the \textit{RealStudent} class (\cdref{realstudent}) for the \textit{Student} corresponding to the given ID's name and returns it (as a String).\\
	\oplabel{getadminname}\opref{getadminname} & GetAdminName & ProxyAdmin & The \textit{GetAdminName} operation is a method offered by the \textit{ProxyAdmin} class (\cdref{proxyadmin}). The operation takes in an ID value (an integer), and asks the \textit{RealAdmin} class (\cdref{realadmin}) for the \textit{Admin} corresponding to the given ID's name and returns it (as a String).\\
	\oplabel{getprojectname}\opref{getprojectname} & GetProjectName & ProxyProject & The \textit{GetProjectName} operation is a method offered by the \textit{ProxyProject} class (\cdref{proxyproject}). The operation takes in an ID value (an integer) and asks the \textit{RealProject} class (\cdref{realproject}) for the \textit{Project} corresponding to the given ID's name and returns it (as a String).\\
