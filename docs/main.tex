...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Exit Conditions &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item The operation will be cancelled and the changes will be reverted.
        \end{itemize} \\
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {>{\bf}l X[2] X[3]}
		...
		Administrator & 
		&
		An \textbf{Administrator} specific type of user that can create/edit projects and run/view results of the PPID\\
		...
		\begin{itemize}[topsep=-1em,leftmargin=*]
			...
			\item Max Size
		\end{itemize} & 
		The \textbf{TeamSize} contains the possible range of team sizes allowed such that the PPID algorithm will not make 
		...
		\begin{itemize}[topsep=-1em,leftmargin=*]
			...
			\item Team Size
		\end{itemize} & 
		A \textbf{Project} contains a collection of Students that will get sorted into Groups. These are made and edited 
		by the \textbf{Administrator}.\\
