...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {>{}l >{\bf}l X}
		...
		UC-10 & FileError & There was an error in processing the file (generalizes FileOpenError, FileInputError,
		FileOutputError, FileFormatError). The system reports the specific error to user and aborts the operation.\\
		UC-11 & FileOpenError & The system reports that the selected file could not be opened.\\
		UC-12 & FileInputError & The system reports that the selected file could not be read.\\
		UC-13 & FileOutputError & The system reports that the selected file could not be written to.\\
		UC-14 & FileFormatError & The system reports that the selected file is corrupted.\\
		UC-15 & InvalidInputError & The system reports that the given value is invalid.\\
		UC-16 & NoStudentSelectedError & The system reports that a student was not selected to carry 
		out the selected operation on.\\
		UC-17 & InsufficientStudentsError & The system reports that there are not enough 
		students registered in project to run the PPID algorithm.\\
