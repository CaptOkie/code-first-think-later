...
The below figure and table cover the use cases that concern the Administrator. With a quick glance of the below diagram, we can see that ManageProjects actually covers a 
great many details including EditProject and CreateNewProject! Note that we denote error cases with an <<extend>> relationship.
...
A picture may tell a thousand words, but, for that same reason, can often take a lot of time to fully extract necessary information. The next table contains 
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
	    ...
	    \begin{enumerate}[topsep=-1em, leftmargin=*]
	    		\item[1.] The Student begins the cuPID process.
	    		\begin{enumerate}
		    		\item[2.] The system presents the Student with a menu of two options: Edit their own values for the attributes (include use case {\bf EditPersonalValues}) or edit their desired partner values for the attributes (include use case {\bf EditDesiredValues}).
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Quality Requirements & 
		\begin{itemize}[topsep=-1em,leftmargin=*]
			\item The Groups are printed out in separate, well-defined sections on the screen. 
		\end{itemize}\\
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Quality Requirements &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item Only Students that aren't in the Project already should be in the list.
        \end{itemize} \\
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Quality Requirements &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item Only the Students that are part of the Project should be in the list.
        \end{itemize} \\
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Flow of Events & 
	    \begin{enumerate}[topsep=-1em, leftmargin=*]
		    \item The system displays a menu of personal details that can be edited. The Student can choose to change the answer
		    of any of the Responses listed.
		\end{enumerate} \\
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Flow of Events & 
	    \begin{enumerate}[topsep=-1em, leftmargin=*]
		    \item The system displays a menu of desired details that can be edited. The Student can choose to change the answer
		    to any of the Responses listed.
		\end{enumerate} \\
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Flow of Events & 
	    \begin{enumerate}[topsep=-1em,leftmargin=*]
		    \item[1.] The Administrator inputs a new project name.
		    \begin{enumerate}
		        \item[2.] The system overwrites the old value with the new value.
		    \end{enumerate}
		\end{enumerate} \\

		Entry Conditions &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item The Administrator has chosen to edit existing, or set the initial, project name.
        \end{itemize} \\

		Exit Conditions &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item The old value is replaced with the new value.
        \end{itemize} \\

		Quality Requirements &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item The project name needs to be unique.
        \end{itemize} \\
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Flow of Events & 
	    \begin{enumerate}[topsep=-1em, leftmargin=*]
		    \item[1.] The Student attempts to join a project.
		    \begin{enumerate}
		    	\item[2.] The system attempts to access the selected project from storage.
		    	\item[3.] If the selected project does not exist, then the system notifies the Student that there was an error with regards to 
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Flow of Events & 
	    \begin{enumerate}[topsep=-1em,leftmargin=*]
		    \item The system notifies the Administrator that an erorr has occured with regards to the reading of a file from storage.
		\end{enumerate} \\

		Entry Conditions &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item An attempt to read a file has failed.
        \end{itemize} \\
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Flow of Events & 
	    \begin{enumerate}[topsep=-1em,leftmargin=*]
		    \item The system notifies the Administrator that an error has occured with regards to the opening of a file from storage.
		\end{enumerate} \\

		Entry Conditions &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item An attempt to open a file has failed.
        \end{itemize} \\
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Flow of Events & 
	    \begin{enumerate}[topsep=-1em,leftmargin=*]
		    \item The system notifies the Administrator that an error has occured with regards to the writing (saving) of a file to storage.
		\end{enumerate} \\

		Entry Conditions &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item An attempt to write/save a file has failed.
        \end{itemize} \\
...
\begin{center}
	\begin{tabu} to \textwidth {>{\it}l X}
		...
		Flow of Events & 
	    \begin{enumerate}[topsep=-1em,leftmargin=*]
		    \item The system notifies the Administrator that an error has occured with regards to the closing of a file.
		\end{enumerate} \\

		Entry Conditions &
		\begin{itemize}[topsep=-1em,leftmargin=*]
		    \item An attempt to close a file has failed.
        \end{itemize} \\
...
\begin{table}
	...
	\begin{tabu} to \textwidth {>{\bf}l X[2] X[3]}
		\tableheader{}Entity Object & Attributes/Associations & Definition\\
		User & 
		\begin{itemize}[topsep=-1em,leftmargin=*]
			\item Name
			\item ID
		\end{itemize} & 
		A {\bf{User}} that has the ability to access the cuPID system.\\
		Student & 
		\begin{itemize}[topsep=-1em,leftmargin=*]
			\item Student Number
			\item Profile
		\end{itemize} & 
		A {\bf{Student}} is a specific type of user that can create/edit their profiles and join/leave existing projects.\\
		Administrator & 
		&
		An {\bf{Administrator}} specific type of user that can create/edit projects and run/view results of the PPID\\
		Group & 
		\begin{itemize}[topsep=-1em,leftmargin=*]
			\item Members
		\end{itemize} & 
		A {\bf{Group}} is a collection of Student users that are participating in a specified project. Every project contains at 
		least 1 group, and the Students in a group are decided when the Administrator runs the PPID algorithm.\\
		TeamSize & 
		\begin{itemize}[topsep=-1em,leftmargin=*]
			\item Min Size
			\item Max Size
		\end{itemize} & 
		The {\bf{TeamSize}} contains the possible range of team sizes allowed such that the PPID algorithm will not make 
		too big or too small groups.\\
		Project & 
		\begin{itemize}[topsep=-1em,leftmargin=*]
			\item Name
			\item Students
			\item Team Size
		\end{itemize} & 
		A {\bf{Project}} contains a collection of Students that will get sorted into Groups. These are made and edited 
		by the {\bf{Administrator}}.\\
		Profile & 
		\begin{itemize}[topsep=-1em,leftmargin=*]
			\item Details
		\end{itemize} & 
		A {\bf{Profile}} is a set of {\bf{Responses}} maintained by each Student. These values are the necessary input 
		for the PPID algorithm to run.\\
		Response & 
		\begin{itemize}[topsep=-1em,leftmargin=*]
			\item Question
			\item Answer
		\end{itemize} & 
		The {\bf{Response}} is a pair of Question and Answer.\\
