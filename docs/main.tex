...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l >{\bf}l X}
		...
		& EditTeamSize & The Administrator sets the minimum and maximum team sizes to be used by the PPID. \\
		& IllegalMinError & The system reports that the given minimum team size is illegal.\\
		& IllegalMaxError & The system reports that the given maximum team size is illegal.\\
	\end{tabu}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabu} to \textwidth {l >{\bf}l X}
	    \tableheader{}ID & Name & Description\\
   		& InvalidProjectError & The system reports that an invalid Project, be it no Project or a non-existent Project, was selected to carry out an operation.\\
		& StorageError &  The system reports that there is an error with the stored information for cuPID. \\
		& StorageReadError & The system reports that the selected file could not be read. \\
		& StorageOpenError & The system reports that the selected file could not be opened. \\
		& StorageWriteError & The system reports that the selected file could not be written to. \\
		& StorageCloseError & The system reports that the selected file could not be closed. \\
	\end{tabu}
\end{table}
